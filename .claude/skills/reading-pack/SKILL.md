...
除了「当日任务包」，还可以出**系列包** —— 一次把一整套书入库，之后由家长在
App 里生成每天的计划。plan 里把 `date` 换成 `series` 就行（两者互斥）：

```json
{
  "series": "牛津树自然拼读 Stage 3",
  "note": "整套入库，不绑定日期",
  "pieces": [ … 按难度**从易到难**排 … ]
}
```

- 每篇会自动带上 `seq`（就是在 plan 里的位置，1 起）。App 生成计划时
  「新书挑最简单的」靠它排序 —— **所以 plan 里的顺序必须是难度顺序**。
