...
- **翻页以左右滑动为主，箭头是辅助。两个页面都要接** —— `Read`（阅读）和
  `Review`（书架进去的复习）是两个独立组件、各有一套箭头，只改一个会漏（已经漏过一次）。 孩子的小手点不准贴边的小箭头，
  而屏幕左右边缘（安卓 10+ 各约 20~24dp）是系统返回手势区、底部是 home 手势区 ——
  误触会直接把阅读退出。所以 `lib/swipe.ts` 里**从边缘 44px 内起手的滑动一律忽略**
